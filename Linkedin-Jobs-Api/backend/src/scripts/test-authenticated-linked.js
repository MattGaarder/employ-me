...
const LINKEDIN_URL = 'https://www.linkedin.com/jobs/view/4458091863/';
async function main() {
    ...
    try {
         ...
         */
        const cookiesBeforeNavigation = await page.cookies('https://www.linkedin.com');
        console.log('\nLinkedIn cookies BEFORE navigation:');
        console.log(cookiesBeforeNavigation.map((cookie) => ({
            name: cookie.name,
        ...
        console.log('\nAuthenticated cookie present:', cookiesBeforeNavigation.some((cookie) => cookie.name === 'li_at'));
        /*
        ...
        console.log('Body:', (await page.evaluate(() => document.body?.innerText || '')).slice(0, 3000));
        const cookiesAfterNavigation = await page.cookies('https://www.linkedin.com');
        console.log('\nAuthenticated cookie after navigation:', cookiesAfterNavigation.some((cookie) => cookie.name === 'li_at'));
        /*
        ...
        const diagnostics = await page.evaluate(() => {
             ...
             */
            const applicationAncestors = Array.from(document.querySelectorAll('*'))
                .filter((el) => {
                const htmlEl = el;
                const combined = [
                    el.tagName,
             ...
             * ==============================
             */
            const applicationButtons = buttons.filter((button) => {
                const combined = [
                    button.text,
                    button.ariaLabel,
                    button.role,
             ...
             */
            const applyElements = Array.from(document.querySelectorAll('*'))
                .filter((el) => {
        ...
        console.log('==============================');
        console.log(JSON.stringify(diagnostics.applicationLinks, null, 2));
        console.log('\n==============================');
        console.log('APPLICATION BUTTONS');
        console.log('==============================');
        console.log(JSON.stringify(diagnostics.applicationButtons, null, 2));
        console.log('\n==============================');
        console.log('EXTERNAL LINKS');
        console.log('==============================');
        console.log(JSON.stringify(diagnostics.externalLinks, null, 2));
        console.log('\n==============================');
        console.log('APPLY ELEMENTS');
        console.log('==============================');
        console.log(JSON.stringify(diagnostics.applyElements, null, 2));
        /*
         * Save diagnostic output.
         */
        const outputDir = path_1.default.resolve(process.cwd(), 'logs/linkedin-auth-test');
        await promises_1.default.mkdir(outputDir, {
            recursive: true,
        });
        await promises_1.default.writeFile(path_1.default.join(outputDir, 'diagnostics.json'), JSON.stringify({
